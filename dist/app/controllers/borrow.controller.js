...
const borrowBook = async (req, res) => {
    try {
        ...
        const foundBook = await Book_1.Book.findById(book);
        if (!foundBook) {
        ...
        const borrowRecord = await Borrow_1.Borrow.create({ book, quantity, dueDate });
        res.json({
            success: true,
...
const getBorrowedSummary = async (req, res) => {
    try {
